...
def centers(total): #finds the center of all clusters
    ...
    for cent in centroids: #isolates centroid coordinates
        ...
        centa = cent[0].strip("[]''(),")
        centb = cent[1].strip("[]''(),")
        centt.append(list(zip(float(centa),float(centb)))) #adds cleaned coordinates
    return centt
...
def analyze(right,left): #compares shoe data to find inconsistencies
    ...
    for x in range(clus): #isolates each cluster
        ...
        if (error > 100): #return a problem if shoes are too uneven
...
plt.plot(ldata2,color = "darkblue")
plt.plot(ldata3,color = "darkblue")
plt.show()
